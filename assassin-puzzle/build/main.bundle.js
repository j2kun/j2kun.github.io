...
var Ray = function () {
  function Ray(center, direction) {
    var length = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 100000;
...
function createCircleSVG(point) {
  var circleSVG = svg.datum(point).append('circle').attr("cx", fromCartesianX(point.x)).attr("cy", fromCartesianY(point.y)).attr("r", pointRadius).attr("fill", labelToColor[point.label]).attr("stroke", labelToStrokeColor[point.label]).attr("stroke-width", 2);
  return circleSVG;
}
...
function updateGuardsSVG(guards) {
  var circleContainers = svg.selectAll(".guard").data(guards);

  circleContainers.enter().append('circle').merge(circleContainers).attr("cx", function (d) {
    return fromCartesianX(d.x);
  }).attr("cy", function (d) {
    return fromCartesianY(d.y);
  ...
  }).attr("stroke-width", 2).attr("class", "guard");
...
function setupBehavior(baseObjects, assassinSVGs, guardsSVG, targetSVG) {
  ...
  svg.on('mousemove', function () {
    ...
    if (x != 0 && y != 0) {
      ...
      rayLinesSVG.attr("d", lineFunction(square.rayToPoints(ray, svg.selectAll(".guard").data().concat([targetSVG.datum()]))));
    }
  });
...
var ray = new _geometry.Ray(assassin, new _geometry.Vector(10, 12));
